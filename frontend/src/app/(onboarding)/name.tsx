...
export default function NameScreen() {
  ...
  const logoWidth = Math.max(150, Math.min(shortSide * 0.48, 280));
  const logoHeight = logoWidth * 0.75;
  const titleSize = Math.max(20, Math.min(shortSide * 0.055, 30));
  const inputHeight = Math.max(44, Math.min(height * 0.055, 56));
      ...
      >
        ...
        <TextInput
          style={[
            ...
            {
              height: inputHeight,
              fontSize: Math.max(15, Math.min(shortSide * 0.038, 19)),
            },
          ]}
          value={name}
